...
export function AuthContextProvider({ children }: { children: ReactNode }) {
  ...
  const syncUser = useCallback(async () => {
    ...
    if (now - lastSyncAttempt < MIN_SYNC_INTERVAL && hasAttemptedSync) {
      console.warn(`Sync attempted too soon (${Math.round((MIN_SYNC_INTERVAL - (now - lastSyncAttempt)) / 1000)}s remaining), skipping`);
      return;
    }
  ...
  }, [isAuthenticated, user, isSyncing, getAccessToken, lastSyncAttempt, hasAttemptedSync, MIN_SYNC_INTERVAL]);
  ...
  useEffect(() => {
    if (isAuthenticated && user && !isLoading && !hasAttemptedSync && !isSyncing) {
      syncUser(); // Run syncUser only once to prevent infinite sync-retry loop
      setTokenGetter(getAccessToken);
    }
  }, [isAuthenticated, user, isLoading, isLoading, hasAttemptedSync, isSyncing, syncUser, getAccessToken]);
