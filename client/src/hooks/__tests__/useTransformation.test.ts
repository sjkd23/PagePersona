...
describe('hooks/useTransformation', () => {
  ...
  beforeEach(() => {
    ...
    mockApiService.getPersonas.mockResolvedValue({
      ...
      data: {
        personas: [
          createMockPersona('eli5'),
          createMockPersona('pirate')
        ]
      }
    })
  })
  ...
  it('should handle transformation success', async () => {
    ...
    expect(result.current.state.content).toEqual(expect.objectContaining({
  ...
  it('should handle transformation error', async () => {
    ...
    expect(result.current.state.error).toBe('Failed to transform the webpage. Please check your connection and try again.')
    expect(result.current.state.isLoading).toBe(false)
  })
