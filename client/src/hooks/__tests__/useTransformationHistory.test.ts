...
describe("useTransformationHistory", () => {
  ...
  describe("addToHistory", () => {
    ...
    it("should maintain maximum history items limit", () => {
      const existingHistory: HistoryItem[] = Array.from(
        { length: 5 },
        (_, i) => ({
          ...mockWebpageContent,
          id: `id-${i}`,
      ...
      expect(result.current.history[0].originalUrl).toBe(
        "https://new-example.com",
      );
      expect(result.current.history.some((item) => item.id === "id-4")).toBe(
        false,
      );
    });
