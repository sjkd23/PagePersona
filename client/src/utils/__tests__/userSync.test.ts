...
describe('userSync', () => {
  const validToken = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhdXRoMHwxMjMiLCJhdWQiOiJ0ZXN0LWF1ZGllbmNlIiwiZXhwIjo5OTk5OTk5OTk5LCJzY29wZSI6InJlYWQ6cHJvZmlsZSJ9.fake-signature'
  ...
  describe('token validation', () => {
    ...
    it('should return null for expired token', async () => {
      const expiredToken = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhdXRoMHwxMjMiLCJleHAiOjE2MDAwMDAwMDB9.fake-signature'
  ...
  describe('sync flow when profile not found', () => {
    ...
    it('should return null when sync fails', async () => {
      ...
      expect(logger.sync.error).toHaveBeenCalledWith(
        'Sync request failed',
        undefined,
        { status: 500, errorText: 'Internal server error' }
      )
    })
  })
  ...
  describe('error handling', () => {
    it('should handle profile fetch failure', async () => {
      ...
      expect(logger.sync.error).toHaveBeenCalledWith(
        ...
        undefined,
        { status: 500, errorText: 'Server error' }
      )
    })
