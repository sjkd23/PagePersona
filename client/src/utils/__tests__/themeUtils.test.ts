...
import { themeClasses, getComponentClasses, personaColors } from '../themeUtils'
...
describe('utils/themeUtils', () => {
  ...
  describe('personaColors', () => {
    ...
    it('should contain expected persona IDs', () => {
      const expectedPersonas = ['eli5', 'anime', 'knight', 'hacker', 'pirate', 'scientist', 'comedian', 'zen']
    ...
    it('should have unique color schemes for each persona', () => {
      ...
      const uniqueLightBgs = new Set(lightClasses.map(cls => cls.match(/bg-\w+-\d+/)?.[0]))
      ...
      const uniqueDarkBgs = new Set(darkClasses.map(cls => cls.match(/bg-\w+-\d+/)?.[0]))
  ...
  describe('type safety', () => {
    it('should only accept valid component types', () => {
      ...
      const validComponents = ['card', 'modal', 'button', 'input', 'dropdown'] as const
