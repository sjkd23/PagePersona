...
import { z } from 'zod';
import { transformSchemas, userSchemas, chatSchemas, authSchemas } from '../src/middleware/validation-schemas';
...
describe('Zod Validation Schemas', () => {
  ...
  describe('Transform Text Schema', () => {
    it('should pass validation for valid transform text request', () => {
      const validData = {
        text: 'This is a valid text that is longer than 10 characters',
        persona: 'professional',
      };
    ...
    it('should trim whitespace from text', () => {
      const dataWithWhitespace = {
        text: '  This is valid text with whitespace  ',
        persona: 'professional',
      };
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.text).toBe('This is valid text with whitespace');
      }
    });
  });
