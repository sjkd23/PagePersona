...
describe("PromptBuilderService", () => {
  const mockContent: ParsedContent = {
    cleanedText:
      "This is a sample content about quantum physics and how particles behave.",
  ...
  describe("buildTransformationPrompt", () => {
    it("should build a valid transformation prompt for ELI5 persona", () => {
      const result = PromptBuilderService.buildTransformationPrompt(
        mockContent,
        "eli5",
      );
      ...
      expect(result.totalLength).toBe(
        result.systemPrompt.length + result.userPrompt.length,
      );
    });
    ...
    it("should throw error for unknown persona", () => {
      expect(() =>
        PromptBuilderService.buildTransformationPrompt(
          mockContent,
          "unknown-persona",
        ),
    ...
    it("should include content text in the user prompt", () => {
      ...
      expect(result.userPrompt).toContain(
        "quantum physics and how particles behave",
      );
    });
  });
  ...
  describe("buildTextTransformationPrompt", () => {
    it("should build a valid text transformation prompt", () => {
      const result = PromptBuilderService.buildTextTransformationPrompt(
        mockContent,
        "eli5",
      );
    ...
    it("should differentiate between webpage and text content prompts", () => {
      const webpagePrompt = PromptBuilderService.buildTransformationPrompt(
        mockContent,
        "eli5",
      );
      const textPrompt = PromptBuilderService.buildTextTransformationPrompt(
        mockContent,
        "eli5",
      );
  ...
  describe("persona-specific behavior", () => {
    it("should include persona-specific instructions in system prompt", () => {
      const eli5Result = PromptBuilderService.buildTransformationPrompt(
        mockContent,
        "eli5",
      );
      const knightResult = PromptBuilderService.buildTransformationPrompt(
        mockContent,
        "medieval-knight",
      );
