...
describe("ParserService", () => {
  describe("parseWebContent", () => {
    it("should parse valid web content successfully", () => {
      ...
      const content =
        "This is a test article with enough content to meet the minimum requirements. ".repeat(
          5,
        );
    ...
    it("should throw error for content that is too short", () => {
      ...
      expect(() => ParserService.parseWebContent(title, content)).toThrow(
    ...
    it("should handle empty title gracefully", () => {
      const title = "";
      const content =
        "This is sufficient content for the parser to work with. ".repeat(3);
  ...
  describe("parseDirectText", () => {
    it("should parse direct text input successfully", () => {
      const text =
        "This is direct text input that meets the minimum length requirements. ".repeat(
          3,
        );
    ...
    it("should throw error for text that is too short", () => {
      ...
      expect(() => ParserService.parseDirectText(text)).toThrow(
        "Text too short to process",
      );
    });
  ...
  describe("edge cases", () => {
    it("should handle content with only whitespace", () => {
      ...
      expect(() => ParserService.parseDirectText(content)).toThrow(
        "Text too short to process",
      );
    });
