...
describe("TransformationService", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(MockedCachedContentTransformer).mockImplementation(
      () => mockTransformer,
    );
  ...
  describe("transformWebpage", () => {
    it("should return cached result when available", async () => {
      ...
      mockedCacheService.getCachedTransformation.mockReturnValue(
        mockCachedResult,
      );
    ...
    it("should scrape and transform content when not cached", async () => {
      ...
      expect(MockedWebScraper.scrapeWebpage).toHaveBeenCalledWith(
        "https://test.com",
      );
      expect(mockedCacheService.setCachedContent).toHaveBeenCalledWith(
        "https://test.com",
        mockScrapedContent,
      );
      expect(mockedCacheService.setCachedTransformation).toHaveBeenCalledWith(
        "https://test.com",
        "test-persona",
        mockTransformResult,
      );
  ...
  describe("transformText", () => {
    it("should transform text successfully", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockTransformResult);
      expect(mockTransformer.transformText).toHaveBeenCalledWith(
        "test text",
        "test-persona",
      );
      expect(mockedIncrementUserUsage).toHaveBeenCalledWith("user123", {
