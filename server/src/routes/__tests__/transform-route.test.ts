...
vi.mock('../../../shared/constants/personas', () => ({
  ...
  getAllClientPersonas: vi.fn().mockReturnValue([
    { id: 'professional', name: 'Professional' }
  ])
}))
...
vi.mock('../../utils/response-helpers', () => ({
  ...
  sendInternalError: vi.fn((res, error) => res.status(500).json({ success: false, error }))
}))
