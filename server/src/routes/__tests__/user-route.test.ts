...
vi.mock('../../utils/response-helpers', () => ({
  ...
  sendError: vi.fn((res, error) => res.status(500).json({ success: false, error })),
  sendNotFound: vi.fn((res, message) => res.status(404).json({ success: false, message })),
  sendUnauthorized: vi.fn((res, message) => res.status(401).json({ success: false, message })),
}));
...
describe('User Route', () => {
  ...
  describe('PUT /api/user/profile', () => {
    it('should update user profile successfully', async () => {
      const mockUserService = await import('../../services/user-service');
      vi.mocked(mockUserService.userService.updateUserProfile).mockResolvedValue({
        success: true,
        data: {
          _id: 'test-id',
          email: 'test@example.com',
    ...
    it('should handle profile update errors', async () => {
      const mockUserService = await import('../../services/user-service');
      vi.mocked(mockUserService.userService.updateUserProfile).mockRejectedValue(
