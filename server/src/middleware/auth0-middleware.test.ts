...
describe('Auth0 Middleware', () => {
  ...
  describe('Error Handling', () => {
    it('should handle errors gracefully without breaking the request', async () => {
      ...
      await expect(syncAuth0User(mockRequest as Request, mockResponse as Response, mockNext))
    ...
    it('should handle non-user routes appropriately', async () => {
      ...
      expect(vi.mocked(mockNext).mock.calls.length + vi.mocked(mockResponse.status!).mock.calls.length).toBeGreaterThan(0)
    })
  })
