...
export async function incrementUserUsage(
  ...
  try {
    ...
    if (!success) {
      if (logErrors) {
        console.warn(`User not found or update failed for usage tracking: ${userId}`);
      }
      return false;
    }
    ...
    if (logSuccess) {
      ...
      console.log(`Usage tracked for user: ${userId} (${userStats?.monthlyUsage || 0} this month)`);
    }
...
export async function incrementUserUsageByAuth0Id(
  ...
  try {
    ...
    if (!mongoUser) {
      if (logErrors) {
        console.warn(`⚠️ User not found for usage tracking: ${auth0Id}`);
      }
      return false;
    }
    ...
    if (logSuccess) {
      console.log(`✅ Usage tracked for user: ${auth0Id} (${mongoUser.usage.monthlyUsage} this month)`);
    }
...
/**
 * Get usage limit for a user based on their role
 */
...
export async function bulkIncrementUsage(
  ...
  try {
    ...
    if (logSuccess) {
      console.log(`✅ Bulk usage tracked: ${updated}/${userIds.length} users updated`);
    }
    
    return { success: true, updated, total: userIds.length };
  } catch (error) {
    if (logErrors) {
      console.error('❌ Failed bulk usage tracking:', error);
    }
...
export async function incrementUserUsageWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        if (logErrors) {
          console.error(`❌ Failed to track usage after ${maxRetries} attempts:`, error);
        }
