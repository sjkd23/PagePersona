...
describe('auth0-sync', () => {
  ...
  const createMockMongoUser = (overrides: Partial<IMongoUser> = {}): IMongoUser =>
    ({
      _id: 'mongo-id',
      auth0Id: 'auth0|123',
  ...
  describe('syncAuth0Fields', () => {
    it('should sync all changed fields', () => {
      ...
      expect(result.changedFields).toContain('email (test@example.com → new@example.com)');
      expect(result.changedFields).toContain('firstName (Test → NewFirst)');
      expect(result.changedFields).toContain('lastName (User → NewLast)');
      expect(result.changedFields).toContain('avatar (old-avatar.png → new-avatar.png)');
    ...
    it('should capture and continue on field sync errors', () => {
      ...
      expect(result.errors).toContain('Failed to sync field email: Database write error');
  ...
  describe('logSyncResults', () => {
    it('should log changed fields', () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('🔄 User user-123 - Fields updated:', [
        'email (old → new)',
        'firstName (old → new)',
      ]);
    });
    ...
    it('should log sync errors', () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith('⚠️  User user-123 - Sync errors:', [
        'Error 1',
        'Error 2',
      ]);
    });
