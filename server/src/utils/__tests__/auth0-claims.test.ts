...
describe("auth0-claims", () => {
  ...
  const createMockAuth0User = (
    overrides: Partial<Auth0JwtPayload> = {},
  ): Auth0JwtPayload => ({
    sub: "auth0|123456789",
  ...
  describe("safeGetAuth0Claims", () => {
    ...
    it("should throw error for missing auth0User", () => {
      expect(() => safeGetAuth0Claims(null as any)).toThrow(
        "Auth0 user object is required",
      );
    });

    it("should throw error for missing sub claim", () => {
      const auth0User = createMockAuth0User({ sub: undefined as any });
      expect(() => safeGetAuth0Claims(auth0User)).toThrow(
        "Auth0 sub claim is missing - invalid JWT token",
      );
    });
  ...
  describe("validateAuth0User", () => {
    ...
    it("should detect null/undefined auth0User", () => {
      ...
      expect(result1.errors).toContain(
        "Auth0 user object is null or undefined",
      );
    ...
    it("should detect invalid email format", () => {
      ...
      expect(result.errors).toContain(
        "Email claim is present but invalid format",
      );
    });
  });
  ...
  describe("debugAuth0Claims", () => {
    it("should log debug information in non-production environment", () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        "Auth0 Claims Debug for user user-123:",
      );
    ...
    it("should identify unknown claims", () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith("Unknown Claims:", {
