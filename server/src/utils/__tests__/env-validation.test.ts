  ...
  describe("envSchema", () => {
    ...
    it("should throw on missing required variables", () => {
      ...
      if (!result.success) {
        ...
        const mongoIssue = result.error.issues.find((issue) =>
          issue.path.includes("MONGODB_URI"),
        );
    ...
    it("should validate JWT secret minimum length", () => {
      ...
      if (!result.success) {
        const secretIssue = result.error.issues.find((issue) =>
          issue.path.includes("JWT_SECRET"),
        );
  ...
  describe("parsedEnv integration", () => {
    it("should contain all required keys when environment is valid", async () => {
      ...
      process.env.JWT_SECRET =
        "test-jwt-secret-1234567890abcdef-minimum-32-chars";
