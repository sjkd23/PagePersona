...
async function testCachedTransformer() {
  ...
  try {
    ...
    const mockScrapedContent = {
      title: 'Test Article',
      content: 'This is a test article content for testing Redis caching functionality.',
    ...
    try {
      const result1 = await transformer.transformContent(mockScrapedContent, 'professional');
    ...
    console.log('🎯 Test 2: Second transformation (expected cache hit if Redis available)');
    const start2 = Date.now();

    try {
      const result2 = await transformer.transformContent(mockScrapedContent, 'professional');
    ...
    console.log('ℹ️  To test Redis caching, ensure Redis is running on redis://localhost:6379');
  } catch (error) {
